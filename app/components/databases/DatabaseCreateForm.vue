    ...
    <UCard class="xl:col-span-2">
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <UFormField label="Database Name" required>
          <UInput v-model="form.database_name" placeholder="example_app_db" icon="i-lucide-database" class="w-full" />
        </UFormField>
        <UFormField label="Password" required >
          <UInput class="w-full" v-model="form.password" :type="showPassword ? 'text' : 'password'"
            placeholder="Enter strong database password" icon="i-lucide-key-round">
            <template #trailing>
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          <UButton color="neutral" variant="soft" to="/databases">
            Cancel
          </UButton>
